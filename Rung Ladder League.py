...
with open('records.json') as uM:
  records = json.load(uM)
    ...
    if(TeamName not in rTeams):
        ...
        if (role == 'Captain' or role == 'Player'):
            ...
            if(playerRostered == 0):
                ...
                teamRosters[TeamName][player].append(SteamID)
                teamRosters[TeamName][player].append(role)
                teamRosters[TeamName][player].append(str(today.strftime("%m/%d/%Y")))
                print(teamRosters)
                playerAdd = discord.utils.get(ctx.message.guild.roles, name=TeamName)
                await ctx.message.author.add_roles(playerAdd)
...
@client.command(pass_context=True)
async def test(ctx):
    await ctx.send(ctx.message.author.roles)
